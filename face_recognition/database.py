...
class Student(db.Model):
    ...
    enrollment = db.Column(db.String(12), unique=False, nullable=False)
    ...
    div = db.Column(db.String(20), nullable=True, default='G')
...
class user(db.Model,UserMixin):
    ...
    faculty = db.relationship("TimeTable", backref="user")
...
class TimeTable(db.Model):
    ...
    faculty_id = db.Column(db.Integer, db.ForeignKey("user.userid"))
    attendence = db.relationship("Attendence", backref="timetable")


    def __repr__(self):
        return f"Student('{self.subject}','{self.slot}','{self.faculty_name}')"
